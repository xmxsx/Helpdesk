        ...
        <div class="list-container" v-if="!selectedTicket">
          <div v-if="filteredTickets.length === 0" class="no-tickets">
            ...
            <div class="add-ticket-container">
              <p>Add ticket</p>
              <NuxtLink to="/user/addTicket" class="add-ticket-button"><i class="fa-solid fa-circle-plus"></i></NuxtLink>
            </div>
          </div>
          <div v-else>
        ...
        <div v-else class="ticket-details">
          ...
          <div class="details-body">
            <div class="details-column-left">
              ...
              <p><span>Status:</span> {{ selectedTicket.status }}</p>
            </div>
...
const fetchTickets = async (status) => {
  try {
    ...
    tickets.value = data.filter((ticket) => ticket.email === authStore.user.email && 
      (status === 'all' || ticket.status === status));
    filteredTickets.value = tickets.value;
    authStore.saveTickets(tickets.value); // Save tickets in sessionStorage
  } catch (error) {
